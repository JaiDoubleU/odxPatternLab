    ...
    function gaugeChart() {
        var svg,
            margin = {top: 0, right: 65, bottom: 10, left: 65},
            width = 250,
            height = 155,
            arcMin = -Math.PI/2,
            arcMax = Math.PI/2,
            ...
            colorOptions = [colorScheme[0], colorScheme[4], colorScheme[6]],
            arc = d3.arc();
        ...
        function chart(selection) {
            selection.each(function(data) {
                ...
                // Select the gaugeChartSVG element, if it exists.
                svg = d3.select(this).selectAll("svg#animatedGauge").data([data]);
                ...
                arcGEnter.append("path").attr("class", "data-arc")
                    ...
                    .style('fill', colorOptions[dataDomain[0]])
    ...
    function resize(gauge) {
        var gWidth = Math.min(d3.select("#animatedGauge").node().offsetWidth, 260);
        gauge.width(gWidth).innerRadius(gWidth / 4).outerRadius((gWidth / 4) + 40);
        d3.select("#animatedGauge").call(gauge);
    }

$('#animatedGauge').ready(function () {
        gauge = gaugeChart()
            .width(250)
            .height(200);

        d3.select("#animatedGauge").datum([randNumberBounds(0, 100)]).call(gauge);

        //resize(gauge);
        window.addEventListener("resize", resize);

        var button = document.getElementById("randomizeGaugeBtn");
        if(button) {
