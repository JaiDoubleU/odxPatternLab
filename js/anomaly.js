    ...
    var initAnomalyTicketDataTable = function () {
        anomalyTicketTable = $('#anomalyTicketList').DataTable( {
            rowId: 'ticketNumber',
            responsive: true,
            colReorder: true,
            "order": [[ 2, 'asc' ]],
                ...
                },
                {
                    ...
                    "render": {
                        "display": function(data, type, row, meta) {
                            if (row["ticketAlert"] == 'Blue') {
                                return (
                                    '<span class="fa-stack">' +
                                        '<i class="fas fa-shield fa-stack-1x text-info"></i>' +
                                        '<i class="fas fa-info fa-stack-1x fa-xs fa-inverse"></i>' +
                                    '</span>'
                                );
                            } else if (row["ticketAlert"] == 'Yellow') {
                                return (
                                    '<span class="fa-stack">' +
                                    '<i class="fas fa-shield fa-fw fa-stack-1x" style="color: #FFAD30;"></i>' +
                                    '<i class="fas fa-question fa-xs fa-fw fa-stack-1x"></i>'+
                                    '</span>'
                                );
                            } else if (row["ticketAlert"] == 'Red') {
                                return (
                                    '<span class="fa-stack">' +
                                    '<i class="fas fa-shield fa-stack-1x text-danger"></i>' +
                                    '<i class="fas fa-exclamation fa-stack-1x fa-xs fa-inverse"></i>' +
                                    '</span>'
                                );
                            } else if (row["ticketAlert"] == 'Green') {
                                return (
                                    '<span class="fa-stack">' +
                                    '<i class="fas fa-shield fa-stack-1x text-success"></i>' +
            ...
            "ajax": "../../styleguide/data/ticketListPatternData.json",
        ...
        $('#anomalyTicketList tbody').on('click', 'td.details-control', function () {
    ...
    clearSuperTicketFilters = function() {
        $('#anomalyTicketList').dataTable().search("").draw();
    };
    ...
    $('#clearSuperTicketFiltersBtn').on('click', function () {
        anomalyTicketTable.search('').draw();
        setTimeout(function () {
    ...
    $('#superTicketAllocationSummary').on('click', 'tr', function () {
        ...
        if (activeAllocationSummaryRow != 'footerRow') {
            if ( !$('#' + activeAllocationSummaryRow).hasClass('selected')) {
                ...
                anomalyTicketTable.search(searchString).draw();
   ...
   var generateComplianceSummaryTable = function () {
    ...
    $('#superTicketComplianceSummary').on('click', 'td', function () {
        ...
        if (!$('#' + activeComplianceSummaryCell).hasClass('selected')) {
            ...
            // apply the filter on the ticket list
            anomalyTicketTable.search(alert).draw();
        } else {
            setTimeout(function () {
